# M28 — Tiered Refresh & Polling Strategy

> **Status:** 🔜 Planned  
> **Size estimate:** M (1–3 days)  
> **Depends on:** M25.2 ✅ (request coalescing), M25.3 ✅ (lazy Outdated load), M25.4 ✅ (configurable TTL), M26 ✅ (doctor error handling), M27 ✅ (Taps batch)  
...
> **Note:** M25 must be functionally complete before M28 starts, because M28 generalizes the cache TTL and lazy-loading mechanisms introduced in M25.

<!-- See [backlog.md](../backlog.md) B-02 — routed to M25 -->
...
- [ ] M25 is functionally complete (combined Outdated, coalescing, lazy Outdated load, configurable TTL).
- [ ] M26 is complete (`brew doctor` no longer logs false failures).
- [ ] M27 is complete (Taps batch loading).
- [ ] Config fallback chain (M25's `cache_ttl_seconds` / old `auto_refresh_seconds` → M28's `refresh.volatile_seconds`) is accepted.
...
- M25–M27 fix the per-panel call volume, but without a refresh strategy the app still refreshes everything on the same schedule.
...
|---|---|---|---|
| D28-1 | Data classes: volatile, stable, manual. | Per-panel TTL; single TTL. | Three classes are enough to capture real behavior and keep config simple. |
| D28-2 | Lazy panel loading for all non-Status panels. | Keep eager Init fetch. | Avoids work the user never sees. |
| D28-3 | Pause auto-refresh while tasks/modals are active. | Always refresh; cancel refresh on interaction. | Respects foreground work and avoids competing brew locks. |
| D28-4 | Config exposes TTL presets, not per-key knobs. | Expose every cache key separately. | Usable defaults with optional override. |
...
|---|---|---|---|
| **A — Data classes & TTL** | 28.1, 28.2 | Assign TTLs per class, wire config | Unit tests prove TTLs applied |
| **B — Lazy loading** | 28.3 | Fetch only visible panels | E2E confirms no brew calls for hidden panels |
| **C — Refresh behavior** | 28.4, 28.5 | Pause on interaction, manual refresh | Manual smoke checklist passes |
...
|---|---|---|---|---|
| 28.1 | Define data classes and per-class TTLs | S | M25.4 | Cache layer supports per-key TTL presets |
| 28.2 | Wire refresh policy config | S | 28.1 | Config file controls auto-refresh class by class |
| 28.3 | Lazy-load all panels on first focus | S | M25.3 | `Init()` only fetches Status + active panel |
| 28.4 | Pause auto-refresh during tasks/modals | S | 28.3 | No background brew during foreground mutation |
| 28.5 | Manual refresh keys (`r`/`R`) bypass TTL | S | 28.2, 28.3 | Immediate refresh on user request |
| 28.6 | Regression tests & smoke | S | 28.4, 28.5 | All tests pass |

---

## Steps

### 28.1 — Define Data Classes and Per-Class TTLs
...
### 28.2 — Wire Refresh Policy Config
...
**Context:** Users should be able to tune or disable auto-refresh. Replace/extend `AutoRefreshSeconds` and `brew.cache_ttl_seconds` (from M25.4) with class-level controls.
...
brew:
  cache_ttl_seconds: 60     # from M25; maps to refresh.volatile_seconds default
```
...
3. `brew.cache_ttl_seconds` if present (M25 fallback).
...
### 28.3 — Lazy-Load All Panels on First Focus
...
### 28.4 — Pause Auto-Refresh During Tasks/Modals
...
### 28.5 — Manual Refresh Keys Bypass TTL

**Size:** S  
**Phase:** C  
**Depends on:** 28.2, 28.3  
**Blocks:** 28.6
...
|---|---|---|---|
| `TestCacheTTLByDataClass` | unit | 28.1 | Per-class expiry |
| `TestRefreshConfigDisablesAutoRefresh` | unit | 28.2 | Config control |
| `TestRefreshConfigUsesClassIntervals` | unit | 28.2 | TTLs match config |
| `TestLazyLoadAllPanels` | e2e | 28.3 | Hidden panels don't fetch |
| `TestStartupOnlyFetchesStatus` | unit | 28.3 | Minimal startup |
| `TestRefreshPausedDuringTask` | unit | 28.4 | No background fetch during mutation |
| `TestRefreshPausedDuringModal` | unit | 28.4 | No background fetch during modal |
| `TestManualRefreshBypassesCache` | unit | 28.5 | User can force refresh |
| Existing flows | e2e | 28.6 | No regression |
...
- [ ] All steps 28.1–28.6 complete; acceptance criteria checked
...
1. Steps safe to keep independently: 28.1 (TTL infra) is useful on its own.
2. Revert order: 28.5 → 28.4 → 28.3 → 28.2 → 28.1.
