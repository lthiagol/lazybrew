...
> **Enables:** M28 (clean diagnostics tab / status dashboard behavior)  
> **Parallel track:** —  
...
- It is a pure correctness fix with no dependency on larger refactoring (M25–M28).
...
- **Reducing how often doctor runs** — covered by **M28** tiered refresh strategy.
- **Changing doctor parsing format** — keep existing `parseDoctorWarnings`; only exit-code handling changes.
- **Other diagnostics commands** — unless audit in 26.2 reveals a similar pattern.
...
|---|---|---|---|
| D26-1 | `Doctor()` accepts exit code 1 and parses stdout for warnings. | Treat all non-zero exits as errors; skip stderr parsing. | Matches Homebrew CLI contract and user expectation that warnings are visible, not failures. |
...
|---|---|---|---|
| **A — Fix** | 26.1 | Correct doctor exit-code handling | Unit tests prove warnings are parsed despite exit=1 |
| **B — Audit** | 26.2 | Verify no other command has the same pattern | Test/log review signed off |
...
|---|---|---|---|---|
| 26.1 | Accept exit=1 from `brew doctor` | S | — | `Doctor()` returns warnings on exit=1 |
| 26.2 | Audit non-zero exit semantics | S | 26.1 | Document or fix other commands |

---

## Steps

### 26.1 — Accept Exit=1 from `brew doctor`
...
### 26.2 — Audit Non-Zero Exit Semantics
...
|---|---|---|---|
| `TestDoctorExitOneReturnsWarnings` | unit | 26.1 | Warnings visible despite exit=1 |
| `TestDoctorExitTwoReturnsError` | unit | 26.1 | Real errors still propagate |
...
- [ ] All steps 26.1–26.2 complete; acceptance criteria checked
