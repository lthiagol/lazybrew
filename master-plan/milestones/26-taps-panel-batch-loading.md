# M26 — Taps Panel Batch Loading

> **Status:** 🔜 Planned  
> **Size estimate:** S (<1 day)  
> **Depends on:** M22 ✅ (release tag)  
> **Enables:** M27 (lazy panel loading, longer tap TTL)  
> **Parallel track:** —  
...
> **Note:** M24.2 request coalescing is *recommended* but not blocking. If M26 lands before M24, add a follow-up task to adopt coalescing for `KeyTapsList`.
...
- [ ] `brew tap-info --json` output shape has been verified to contain the fields in 26.1 (or the lazy-detail approach in 26.2 is adjusted).
...
- This is the single biggest subprocess cost after `brew doctor` (which is addressed separately in **M25**).
- M24 introduces request coalescing, which makes this refactor safer and easier to test.
...
- **Refresh policy / TTL** — covered by **M27** tiered refresh strategy.
- **Other panels** — Outdated in **M24**, formulae/casks dedupe in **M27**.
...
|---|---|---|---|
| D26-1 | `Taps.List()` calls `tap-info --json` once for all taps. | Sequential per-tap calls; parallel per-tap goroutines. | One subprocess is simplest and fastest; Homebrew already supports it. |
| D26-2 | `FormulaNames`/`CaskNames` are lazy-loaded via `Taps.Get(name)` only for trust menus. | Always include them in list; remove the feature. | Keeps trust menu functional without paying the cost on every startup/refresh. |
...
|---|---|---|---|
| **A — Batch list** | 26.1 | One `tap-info --json` for all taps | Unit test shows one subprocess |
| **B — Lazy detail** | 26.2 | Detail only for trust menu | teatest confirms no per-tap info on startup |
...
|---|---|---|---|---|
| 26.1 | Batch `tap-info` in `Taps.List()` | S | — | Single subprocess tap list |
| 26.2 | Lazy FormulaNames/CaskNames for trust menu | S | 26.1 | Trust menu still works, no detail fetch at list time |
| 26.3 | Regression tests | S | 26.2 | Tests pass |

---

## Steps

### 26.1 — Batch `tap-info` in `Taps.List()`
...
### 26.2 — Lazy FormulaNames/CaskNames for Trust Menu
...
|---|---|---|---|
| `TestTapsListUsesSingleTapInfoCall` | unit | 26.1 | One subprocess for all taps |
| `TestTapsListMapsAllTaps` | unit | 26.1 | Correct mapping |
| `TestTrustMenuLazyLoadsTapNames` | unit/e2e | 26.2 | Trust menu triggers detail fetch |
| `TestTapsListDoesNotFetchNames` | unit | 26.2 | No eager detail fetch |
| Existing tap tests | unit/e2e | 26.3 | No regression |
...
- [ ] All steps 26.1–26.3 complete; acceptance criteria checked
...
1. Steps safe to keep independently: none — 26.2 depends on 26.1.
2. Revert order: 26.2 → 26.1.
